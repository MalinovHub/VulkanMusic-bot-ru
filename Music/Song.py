class Song:
    ...
    def finish_down(self, info: dict) -> None:
        ...
        for key in self.__required_keys:
            if key in info.keys():
                ...
                print(f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> {key} не найден в информации о музыке: {self.identifier}')
                self.destroy()
                return
    ...
    def destroy(self) -> None:
        print(f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> Музыка самоуничтожается {self.__identifier}')
