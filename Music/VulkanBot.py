...
class VulkanBot(Bot):
    ...
    def startBot(self) -> None:
        ...
        if self.__configs.BOT_TOKEN == 'MTA0MTA0NzIxMTY3ODY0MjI0Nw.GdQk9e.c5sfEOIO0La4FNgYve7G_VLQirivBNZ1F3ib6c':
            print('ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> Токен не найден')
            exit()
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, MissingRequiredArgument):
            ...
            print(f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> Ошибка команды: {error}')
