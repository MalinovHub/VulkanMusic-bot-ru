            ...
            with YoutubeDL(options) as ydl:
                try:
                    ...
                    if self.__is_music(extracted_info):
                        ...
                        print(f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> Не удалось извлечь URL {url}')
                        return []
                # Convert the yt_dlp download error to own error
                except DownloadError:
                    raise DownloadingError()
                except Exception as e:
                    print(f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> Ошибка при извлечении музыки: {e}, {type(e)}')
                    raise e
        else:
            return []
    ...
    def __get_forced_extracted_info(self, url: str) -> list:
        ...
        with YoutubeDL(options) as ydl:
            try:
                ...
                print(f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> Ошибка при принудительном извлечении музыки: {e}')
                return []
    ...
    def __download_url(self, url) -> dict:
        ...
        with YoutubeDL(options) as ydl:
            try:
                ...
                print(f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> Ошибка загрузки {url} -> {e}')
                return None
    ...
    async def download_song(self, song: Song) -> None:
        ...
        def __download_func(song: Song) -> None:
            try:
                ...
                print(f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> Ошибка загрузки {song.identifier} -> {e}')
        ...
        with YoutubeDL(options) as ydl:
            try:
                ...
                if self.__is_multiple_musics(extracted_info):
                    ...
                    print(f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> Не удалось извлечь заголовок {title}')
                    return {}
            except Exception as e:
                print(f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> Ошибка загрузки названия {title}: {e}')
                return {}
