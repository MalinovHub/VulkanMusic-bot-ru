...
class VEmbeds:
    ...
    def __addFooterContent(self, embed: Embed) -> Embed:
        footerText = f'\u200b Пожалуйста, поддержите этот проект, поставив звезду: {self.__config.PROJECT_URL}'
    ...
    def BAD_CHOOSE_USE(self) -> Embed:
        embed = Embed(
            title='Выберите что-нибудь',
            description=f'Ошибка: используйте {self.__config.BOT_PREFIX}помощь, чтобы понять эту команду.',
            colour=self.__colors.RED
        )
        return embed
