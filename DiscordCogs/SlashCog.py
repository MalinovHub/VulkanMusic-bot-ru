...
class SlashCommands(Cog):
    ...
    async def play(self, ctx: ApplicationContext,
                   music: Option(str, "Название музыки или URL", required=True)) -> None:
        ...
        try:
            ...
            print(f'[ОШИБКА В КОМАНДЕ] -> {traceback.format_exc()}')
    ...
    async def move(self, ctx: ApplicationContext,
                   from_pos: Option(int, "Позиция песни для перемещения", min_value=1),
                   to_pos: Option(int, "Позиция для размещения песни, по умолчанию 1", min_value=1, по умолчанию=1)) -> Нет:
        if not self.__bot.listingSlash:
            return
        try:
            await ctx.defer()
    ...
    async def remove(self, ctx: ApplicationContext,
                     position: Option(int, "Позиция песни, которую нужно удалить", min_value=1)) -> None:
        if not self.__bot.listingSlash:
            return
        try:
            await ctx.defer()
