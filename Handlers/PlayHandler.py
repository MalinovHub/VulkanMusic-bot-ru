    ...
    async def run(self, track: str) -> HandlerResponse:
        ...
        try:
            ...
            if isinstance(error, VulkanError):  # If error was already processed
                print(
                    f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -s> Ошибка PlayController: {traceback.format_exc()}', {type(error)})
                embed = self.embeds.CUSTOM_ERROR(error)
            else:
                print(
                    f'ПРИМЕЧАНИЕ РАЗРАБОТЧИКА -> Ошибка PlayController: {traceback.format_exc()}, {type(error)}')
                error = UnknownError()
